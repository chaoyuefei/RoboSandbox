...
import numpy as np
import pandas as pd
from robosandbox.performance.workspace.indice_manager import RobotIndices, IndiceRegistry, IndiceManager
...
class TestRobotIndices(unittest.TestCase):
    """Test cases for the RobotIndices class."""
    ...
    def test_yoshikawa(self):
        ...
        result = RobotIndices.yoshikawa(self.workspace, self.joint_points)
        self.assertIsInstance(result, np.ndarray)
        self.assertEqual(len(result), 3)
    
    def test_invcondition(self):
        """Test the inverse condition number index calculation."""
        result = RobotIndices.invcondition(self.workspace, self.joint_points)
        self.assertIsInstance(result, np.ndarray)
        self.assertEqual(len(result), 3)
    
    def test_asada(self):
        """Test the Asada index calculation."""
        result = RobotIndices.asada(self.workspace, self.joint_points)
        self.assertIsInstance(result, np.ndarray)
        self.assertEqual(len(result), 3)
    
    def test_calculate_manipulability_no_robot(self):
        """Test error handling when no robot is set."""
        workspace_no_robot = MockWorkspace(None)
        with self.assertRaises(ValueError):
            RobotIndices._calculate_manipulability(workspace_no_robot, self.joint_points, "yoshikawa")
...
class TestIndiceRegistry(unittest.TestCase):
    ...
    def test_get_function(self):
        ...
        self.assertEqual(function, RobotIndices.yoshikawa)
...
class TestIndiceManager(unittest.TestCase):
    ...
    def test_calculate(self):
        ...
        # Test error for non-existent indice
        with self.assertRaises(ValueError):
            self.manager.calculate("nonexistent_indice", self.workspace, self.joint_points)
