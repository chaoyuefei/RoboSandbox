...
class Variable:
    ...
    def __le__(self, other):
        return Constraint('ineq', Expression('-', self, self))
...
class Opti:
    ...
    def variable(self, init_guess=0, name=None, bounds=(None, None)):
        """
    ...
    def solve(self, method='SLSQP'):
        ...
        if self.objective_sense == 'min':
            obj_func = self.objective.to_callable(self)
        elif self.objective_sense == 'max':
            # To maximize, minimize the negative
            def obj_func(x):
                return -self.objective.evaluate(x, self.parameters)
        else:
    ...
    # Optionally, add methods to set parameters if needed
    def parameter(self, name, value):
        """
        Define a parameter for the optimization problem.
